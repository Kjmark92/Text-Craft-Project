...
This basic analysis is a reproduction of the one performed [here](https://towardsdatascience.com/beginners-guide-to-lda-topic-modelling-with-r-e57a5a8e7a25) but using our Cirque du Soleil data.

#### Load the data:

I'll use the one that's already created about Cirque du Soleil shows.
```{r}
...
#Add id column
Cirque_Train <- Cirque_Train %>% mutate(id = row_number())
...
text_cleaning_tokens <- Cirque_Train %>% 
...
#Adjust the code to include response varaible
#tokens <- tokens %>% 
#  select(id,word,id) %>%
#  group_by(id) %>% 
#  mutate(ind = row_number()) %>%
